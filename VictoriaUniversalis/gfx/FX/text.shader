...
PixelShader = 
{
	...
	[[
		float4 main( VS_FLAG_OUTPUT v ) : PDX_COLOR
		{
			...
			if( FieldCutoff == 0.5 ) { // aka if flag is colonial
				...
				float cantonCoordX = v.vTexCoord.x + v.vTexCoordFlag.x * zoomFactor;
				float cantonCoordY = v.vTexCoord.y + v.vTexCoordFlag.y * zoomFactor;
				float2 cantonTexCoord = float2( cantonCoordX, cantonCoordY );
				
				TexColor = tex2D( SimpleTexture, cantonTexCoord);
				
				if( v.vTexCoordFlag.x > FieldCutoff || v.vTexCoordFlag.y > FieldCutoff )
	...
	[[
		float4 main( VS_FLAG_OUTPUT v ) : PDX_COLOR
		{
			...
			float zoomFactor = 0.1; // zoomFactor needs to be different for revolutionary flags for some reason
			float cantonCoordX = v.vTexCoord.x + v.vTexCoordFlag.x * zoomFactor;
			float cantonCoordY = v.vTexCoord.y + v.vTexCoordFlag.y * zoomFactor;
			float2 cantonTexCoord = float2( cantonCoordX, cantonCoordY );

			float4 OutColor = tex2D( SimpleTexture, cantonTexCoord );
			OutColor = OutColor.r * CustomColor0 + OutColor.g * CustomColor1 + OutColor.b * CustomColor2;
